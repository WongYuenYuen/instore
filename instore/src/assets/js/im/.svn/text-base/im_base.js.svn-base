...
function onSendMsg() {
	...
	var localuser =  Common.getuser();    
	var nickname = localuser.nickname;
    var msg = new webim.Msg(selSess, isSend, seq, random, msgTime, loginInfo.identifier, subType,nickname); 
...
function selectEmotionImg(selImg) {
    $("#txt-ask").val($("#txt-ask").val() + selImg.id);
}
...
//点击登录按钮(独立模式) 
function independentModeLogin(login_account,login_pwd) { 
    loginInfo.identifier = login_account;
    loginInfo.userSig = login_pwd;  
    sdkLogin();
}
	...
	function showMsg(msg) {
	    ...
	    switch (subType) {
	        ...
	        case webim.GROUP_MSG_SUB_TYPE.COMMON: //群普通消息 
	            ...
	            var msgdata = convertMsgtoHtml(msg); 
	             if(livecount==fromAccount){
	            	 
	            	 /*****主播消息*****/ 
	            	 //如果是图片消息 
	            	 if(imimgtype==1){ 
	            		 imimgtype =0; 
	            		 showhtml = '<li><p class="msg-common imhost"><strong>'+fromAccountNick+':</strong><label><img class="gift hostimg" onclick="showimimg(\''+msgdata.getUrl()+'\')" src="'+msgdata.getUrl()+'" /></label></p></li>'; 
	            	 }else{
	            		
		            	  showhtml = '<li><p class="msg-master imhost"><strong>'+fromAccountNick+':</strong><label>'+msgdata+'</label></p></li>'; 
	            	 }
	            	  
	             }else if(sysmsgtype==1){
	            	 
	            	  //系统消息
	            	  var actiontype = getCustomMsgType(msg); 
	            	
	            	  if(actiontype=="like"){ 
	            		  //点赞
	            		  fromAccountNick =   msgdata.data.username;
	            		  //showhtml = '<li><p class="msg-common imlike"><strong>'+fromAccountNick+':</strong><label>送出一个<img class="gift" src="img/gift/gift-like.png" /></label> </p></li>'; 
	            		  flyGifts(); 
	            		  $("#favlike").html(msgdata.data.total);
            	    	  $("#favlike").show();
	            	  }else if(actiontype=='joingroup'){  
	            		  
	            		  fromAccountNick =   msgdata.data.username; 
	            		  //入群消息
		            	  showhtml = '<li><p class="msg-sys imjoin"><strong>【'+fromAccountNick+'】</strong><label>进入直播间</label></p></li>'; 
		            	  updateLiveNum();
	            	  }else if(actiontype=='exitgroup'){  
	            		  
	            		  updateLiveNum();
	            	  }else if(actiontype=='close'){
	            		  
	            		  //直播关闭 
	            		  backpage = "evaluate.html?hostid="+hostid+"&roomid="+roomid+"&shopid="+shopid;
	            		  Common.alert("本次直播结束，感谢观看");  
	            		  setTimeout('outRoom()',3000);
	            		  
	            	  }else if(actiontype=='give'){
	            		  
	            		  //用户赠送礼物
	            		  fromAccountNick =   msgdata.data.username; 
	            		  var gifturl =  msgdata.data.gifturl;
	            		  showhtml = '<li><p class="msg-common imgift"><strong>'+fromAccountNick+':</strong><label>送出一个<img class="gift" src="'+gifturl+'" /></label> </p></li>'; 
	            		 ...
	            		 var userid =  msgdata.data.userid; 
	            		  if(user.id==userid){
	            			 ...
	            			 Common.alert("您已被禁止观看本场直播"); 
	            			 backpage = "host.html?hostid="+hostid;
	            			 setTimeout('outRoom()',2000);
	            		  } 
	            	  }
	            	 ...
	            	 //观众消息
		            showhtml = '<li><p class="msg-common impeople"><strong>'+fromAccountNick+':</strong><label>'+msgdata+'</label></p></li>';    
	             }
	    ...
	    if(showhtml==""){ 
	    	 ...
	    	 $("#msgcontext").append(showhtml);
	    }
	  
	    $('#msgcontext').scrollTop( $('#msgcontext')[0].scrollHeight );
	    //$("#msgcontext").scrollTop = div.scrollHeight;
	}
	  ...
	  for (var i in elems) {
	      ...
	      switch (type) { 
	          ...
	          case webim.MSG_ELEMENT_TYPE.IMAGE: 
	        	  ...
	        	  //消息类型为图片
	        	  imimgtype = 1;  
	              var smallImage = content.getImage(webim.IMAGE_TYPE.LARGE); //小图 
	              return smallImage;
	              break;
	          ...
	          case webim.MSG_ELEMENT_TYPE.CUSTOM:
	          	//后台通过json发送
	              //html += "json格式："+convertCustomMsgToHtml(content);
	              sysmsgtype = 1;
	              return convertCustomMsgToHtml(content);
	...
	function getCustomMsgType(msg) {  
		  ...
		  for (var i in elems) {
	        ...
	        if(type==webim.MSG_ELEMENT_TYPE.CUSTOM){ 
	    	    ...
	    	    var ext = content.getExt(); 
	    	    var jsondata =  JSON.parse(data); 
	    	    result = jsondata.data.text;
	    	    msgtype  = jsondata.action;
	        } 
		  } 
